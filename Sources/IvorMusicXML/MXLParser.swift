...
extension MXLParser {
    ...
    /// Parses a MusicXML document from the provided data and returns the
    /// resulting entity.
    ///
    ...
    ///
    /// - Returns:  The ``MXLEntity`` parsed from the provided data.
    ///
    /// - Throws:   ``MXLParseError`` if the data cannot be parsed.
    public func parse(_ data: Data,
        ...
        if compressed {
            ...
            default:
                throw MXLParseError.parseFailure(nil)
            }
        }
    ...
    private static let pitchAccidentals: [Int: MXLPitch.Accidental] = [-2: .doubleFlat,
                                                                       -1: .flat,
                                                                       0: .natural,
                                                                       1: .sharp,
                                                                       2: .doubleSharp]

    private static let pitchLetters: [String: MXLPitch.Letter] = ["A": .a,
                                                                  "B": .b,
                                                                  "C": .c,
                                                                  "D": .d,
                                                                  "E": .e,
    ...
    private static let ties: [String: MXLTie] = ["start": .start,
                                                 "stop": .stop]
    ...
    private static func _makeDivisions(_ text: String) -> UInt? {
        if let divisions = UInt(text),
           divisions > 0 {
            divisions
        } else {
            nil
    ...
    private static func _makeDuration(_ text: String) -> UInt? {
        if let duration = UInt(text),
           duration > 0 {
            duration
        } else {
            nil
    ...
    private static func _makePercent(_ text: String) -> Float? {
        if let percent = Float(text),
           percent > 0 {
            percent
        } else {
            nil
    ...
    private static func _makePitchLetter(_ text: String) -> MXLPitch.Letter? {
        pitchLetters[text]
    }

    private static func _makePitchOctave(_ text: String) -> MXLPitch.Octave? {
        if let octave = MXLPitch.Octave(text),
           (0...9).contains(octave) {
            octave
        } else {
            nil
    ...
    private static func _makeRealTimeDivisions(_ text: String) -> Float? {
        if let divisions = Float(text),
           divisions > 0 {
            divisions
        } else {
            nil
    ...
    private static func _parse(_ data: Data) throws -> MXLEntity {
        let rootNode = try BaseParser().parse(data)
    ...
    private static func _parseGraceDuration(_ node: Node) throws -> MXLGraceDuration {
        ...
        if let value = try node.valueOfOptionalAttribute(.makeTime, { _makeRealTimeDivisions($0) }) {
    ...
    private static func _parsePitch(_ node: Node) throws -> MXLNote.Value {
        ...
        let letter = try node.valueOfRequiredChildElement(.step) { _makePitchLetter($0) }

        let accidental = try node.valueOfOptionalChildElement(.alter) { _makePitchAccidental($0) }

        let octave = try node.valueOfRequiredChildElement(.octave) { _makePitchOctave($0) }

        return .pitch(MXLPitch(letter: letter,
    ...
    private static func _parseTie(_ node: Node) throws -> MXLTie {
        ...
        return try node.valueOfRequiredAttribute(.type) { _makeTie($0) }
    }
    ...
    private static func _readRootData(from file: FileWrapper,
        ...
        else { throw MXLParseError.noRootFileFound }

        guard Self.validMediaTypes.contains(rootFile.mediaType)
        else { throw MXLParseError.invalidRootFileMediaType(rootFile.mediaType) }
