...
extension MXLDegree {
    ...
    /// The degree-alter type represents the chromatic alteration for the current degree. If the degree-type value is
    /// alter or subtract, the degree-alter value is relative to the degree already in the chord based on its kind
    /// element. If the degree-type value is add, the degree-alter is relative to a dominant chord (major and perfect
    /// intervals except for a minor seventh). The plus-minus attribute is used to indicate if plus and minus symbols
    /// should be used instead of sharp and flat symbols to display the degree alteration. It is no if not specified.
    ///
    /// Maps the MusicXML `<degree-alter>` element.
    public struct Alter {

        // MARK: Public Initializers

        /// Creates a new `MXLDegree.Alter`.
        ///
        ...
        /// - Parameter usesPlusMinus:  A Boolean value indicating whether plus and minus symbols should be used instead of sharp and flat symbols to display the degree alteration. `false` if not specified.
        public init(value: Double,
                    position: MXLPosition = MXLPosition(),
                    font: MXLFont = MXLFont(),
                    color: MXLColor? = nil,
                    usesPlusMinus: Bool? = nil) {
            self.color = color
            self.font = font
            self.usesPlusMinus = usesPlusMinus
        ...
        /// A Boolean value indicating whether plus and minus symbols should be used instead of sharp and flat symbols
        /// to display the degree alteration. `false` if not specified.
        ///
        /// Maps to the MusicXML `plus-minus` attribute.
        public let usesPlusMinus: Bool?
...
extension MXLDegree.Alter: Equatable {
}

// MARK: - Hashable

extension MXLDegree.Alter: Hashable {
}

// MARK: - Sendable

extension MXLDegree.Alter: Sendable {
}
