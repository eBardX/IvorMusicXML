...
/// The heel and toe elements are used with organ pedals. The substitution value is “no” if the attribute is not
/// present.
///
/// Maps the MusicXML `<heel>` and `<toe>` elements.
public struct MXLHeelToe {

    // MARK: Public Initializers

    /// Creates a new `MXLHeelToe`.
    ///
    ...
    /// - Parameter isSubstitute:  A Boolean value indicating whether this heel or toe technique substitutes for the other. `false` if not specified.
    public init(position: MXLPosition = MXLPosition(),
        ...
        self.isSubstitute = isSubstitute ?? false
    }
    ...
    /// A Boolean value indicating whether this heel or toe technique substitutes for the other. `false` if not
    /// specified.
    ///
...
extension MXLHeelToe: Equatable {
}

// MARK: - Hashable

extension MXLHeelToe: Hashable {
}

// MARK: - Sendable

extension MXLHeelToe: Sendable {
}
