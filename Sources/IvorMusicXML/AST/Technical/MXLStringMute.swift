...
///
/// Maps the MusicXML `<string-mute>` element.
public struct MXLStringMute {

    // MARK: Public Initializers

    /// Creates a new `MXLStringMute`.
    ///
    ...
    /// - Parameter isOn:      A Boolean value indicating whether the string mute is on (`true`) or off (`false`).
    ...
    /// A Boolean value indicating whether the string mute is on (`true`) or off (`false`).
    ///
...
extension MXLStringMute: Equatable {
}

// MARK: - Hashable

extension MXLStringMute: Hashable {
}

// MARK: - Sendable

extension MXLStringMute: Sendable {
}
