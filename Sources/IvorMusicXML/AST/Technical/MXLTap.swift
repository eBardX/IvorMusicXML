...
/// The tap type indicates a tap on the fretboard. The text content allows specification of the notation; + and T
/// are common choices. If the element is empty, the hand attribute is used to specify the symbol to use. The hand
/// attribute is ignored if the tap glyph is already specified by the text content. If neither text content nor
/// the hand attribute are present, the display is application-specific.
///
/// Maps the MusicXML `<tap>` element.
public struct MXLTap {

    // MARK: Public Initializers

    /// Creates a new `MXLTap`.
    ///
    /// - Parameter value:      The value of this element.
    /// - Parameter hand:       Specifies the symbol to use for the tap when the element content is empty.
    ...
    /// Specifies the symbol to use for the tap when the element content is empty.
    ///
    /// Maps to the MusicXML `hand` attribute.
...
extension MXLTap: Equatable {
}

// MARK: - Hashable

extension MXLTap: Hashable {
}

// MARK: - Sendable

extension MXLTap: Sendable {
}
