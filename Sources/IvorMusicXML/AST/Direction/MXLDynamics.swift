...
/// Dynamics can be associated either with a note or a general musical direction. To avoid inconsistencies between
/// and among the letter abbreviations for dynamics (what is sf vs. sfz, standing alone or with a trailing
/// dynamic that is not always piano), we use the actual letters as the names of these dynamic elements. The
/// other-dynamics element allows other dynamic marks that are not covered here. Dynamics elements may also be
...
/// representation is inconsistent in scores. Many things are assumed by the composer and left out, such as
/// returns to original dynamics. The MusicXML format captures what is in the score, but does not try to be
/// optimal for analysis or synthesis of dynamics.
///
/// The placement attribute is used when the dynamics are associated with a note. It is ignored when the dynamics
/// are associated with a direction. In that case the direction element’s placement attribute is used instead.
///
/// Maps the MusicXML `<dynamics>` element.
public struct MXLDynamics {

    // MARK: Public Initializers

    /// Creates a new `MXLDynamics`.
    ///
    ...
    /// - Parameter items: One of the alternatives for Dynamics.
...
extension MXLDynamics: Equatable {
}

// MARK: - Hashable

extension MXLDynamics: Hashable {
}

// MARK: - Sendable

extension MXLDynamics: Sendable {
}
