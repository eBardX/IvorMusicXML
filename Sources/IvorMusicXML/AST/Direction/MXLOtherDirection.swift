...
/// The other-direction type is used to define any direction symbols not yet in the MusicXML format. The smufl
/// attribute can be used to specify a particular direction symbol, allowing application interoperability without
/// requiring every SMuFL glyph to have a MusicXML element equivalent. Using the other-direction type without the
/// smufl attribute allows for extended representation, though without application interoperability.
///
/// Maps the MusicXML `<other-direction>` element.
public struct MXLOtherDirection {

    // MARK: Public Initializers

    /// Creates a new `MXLOtherDirection`.
    ///
...
extension MXLOtherDirection: Equatable {
}

// MARK: - Hashable

extension MXLOtherDirection: Hashable {
}

// MARK: - Sendable

extension MXLOtherDirection: Sendable {
}
