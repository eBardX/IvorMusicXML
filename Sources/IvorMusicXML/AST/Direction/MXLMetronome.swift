...
/// The metronome type represents metronome marks and other metric relationships. The beat-unit group and per-
/// minute element specify regular metronome marks. The metronome-note and metronome-relation elements allow for
/// the specification of metric modulations and other metric relationships, such as swing tempo marks where two
/// eighths are equated to a quarter note / eighth note triplet. Tied notes can be represented in both types of
/// metronome marks by using the beat-unit-tied and metronome-tied elements. The parentheses attribute indicates
/// whether or not to put the metronome mark in parentheses; its value is no if not specified. The print-object
/// attribute is set to no in cases where the metronome element represents a relationship or range that is not
/// displayed in the music notation.
///
/// Maps the MusicXML `<metronome>` element.
public struct MXLMetronome {

    // MARK: Public Initializers

    /// Creates a new `MXLMetronome`.
    ///
    ...
    /// - Parameter content: One of the alternatives for Metronome.
    ...
    /// - Parameter hasParentheses: A Boolean value indicating whether to enclose the metronome mark in parentheses. Defaults to `false` if not specified.
    ...
    /// A Boolean value indicating whether to enclose the metronome mark in parentheses. Defaults to `false` if not
    /// specified.
    ///
...
extension MXLMetronome: Equatable {
}

// MARK: - Hashable

extension MXLMetronome: Hashable {
}

// MARK: - Sendable

extension MXLMetronome: Sendable {
}
