...
extension MXLTiedTests {
    @Test
    func equality() {
        #expect(MXLTied(kind: .start) == MXLTied(kind: .start))    // swiftlint:disable:this identical_operands
    }

    @Test
    func hashable() {
        let values: Set<MXLTied> = [MXLTied(kind: .start), MXLTied(kind: .start), MXLTied(kind: .stop)]
    ...
    @Test
    func inequality() {
        #expect(MXLTied(kind: .start) != MXLTied(kind: .stop))
    }

    @Test
    func init_defaultValues() {
        let value = MXLTied(kind: .start)
